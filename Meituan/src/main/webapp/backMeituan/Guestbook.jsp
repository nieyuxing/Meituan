...
<div class="Guestbook_list">
	<table class="table table-striped table-bordered table-hover"
		...
		<thead>
			<tr>
				<th width="25"><label><input type="checkbox"
						name="checkbox" class="ace"/><span class="lbl"/></label></th>
				<th width="80">ID</th>
		...
		<tbody>
			<c:forEach items="${evaluates }" var="assess">
				<tr>
					<td><label><input type="checkbox" name="checkbox"
							class="ace" value="${assess.eid }"/><span class="lbl"></span></label></td>
					<td>${assess.eid }</td>
					<td><u style="cursor: pointer" id="cname" class="text-primary"
						onclick="member_show('张小泉','member-show.html','1031','500','400')"></u>${assess.uname }</td>
					<td class="text-l"><a href="javascript:;"
						onclick="Guestbook_iew('${assess.eid }')">${assess.econtent }</a></td>
						<td>${assess.edate }</td>
						<td class="td-manage"><a
							onclick="Guestbook_iew('${assess.eid }')" title="回复"
							href="javascript:;" class="btn btn-xs btn-info"><i
								class="fa fa-edit bigger-120"></i></a> <a href="javascript:;"
							onclick="member_del(this,'${assess.eid }')" title="删除"
...
function searchAssess() {
	...
	$.post("../assess/searchAssess",{ascount : ascount,date : date},function(data) {
		for (var i = 0; i < data.length; i++) {
			...
			str += 'onclick="member_show()"></u>'+ data[i].cname + '</td>';
